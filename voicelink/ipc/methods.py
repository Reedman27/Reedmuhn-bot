...
from voicelink import Player, Track, Playlist, NodePool, LoopType, Filters, Config, SQLiteMusicDB, LangHandler, LYRICS_PLATFORMS
...
async def connect_channel(member: Member, bot: commands.Bot) -> Player:
    ...
    try:
        settings = await SQLiteMusicDB.get_settings(channel.guild.id)
...
async def initUser(bot: commands.Bot, data: Dict) -> Dict:
    user_id = int(data.get("userId"))
    data = await SQLiteMusicDB.get_user(user_id)
...
async def updateVolume(player: Player, member: Member, data: Dict) -> None:
    ...
    await SQLiteMusicDB.update_settings(player.guild.id, {"$set": {"volume": volume}})
...
async def _getPlaylist(user_id: int, playlist_id: str) -> Dict:
    playlists = await SQLiteMusicDB.get_user(user_id, d_type="playlist")
    playlist = playlists.get(playlist_id)
    if not playlist:
        return
    
    if playlist["type"] == "share":
        target_user = await SQLiteMusicDB.get_user(playlist["user"], d_type="playlist")
...
async def updatePlaylist(bot: commands.Bot, data: Dict) -> Dict:
    ...
    if _type == "createPlaylist":
        ...
        playlist = await SQLiteMusicDB.get_user(user_id, d_type="playlist")
        ...
        await SQLiteMusicDB.update_user(user_id, {"$set": {f"playlist.{assigned_playlist_id}": data}})
        return {
            "op": "updatePlaylist",
            "status": "created",
        ...
        if playlist:
            if playlist['type'] == 'share':
                await SQLiteMusicDB.update_user(playlist['user'], {"$pull": {f"playlist.{playlist['referId']}.perms.read": user_id}})

            await SQLiteMusicDB.update_user(user_id, {"$unset": {f"playlist.{playlist_id}": 1}})
        ...
        await SQLiteMusicDB.update_user(user_id, {"$set": {f'playlist.{playlist_id}.name': name}})
        return {
            "op": "updatePlaylist",
            "status": "renamed",
            "name": name,
        ...
        await SQLiteMusicDB.update_user(user_id, {"$push": {f'playlist.{playlist_id}.tracks': track_id}})
        return {
            "op": "updatePlaylist",
            "status": "addTrack",
            "playlistId": playlist_id,
        ...
        await SQLiteMusicDB.update_user(user_id, {"$pull": {f'playlist.{playlist_id}.tracks': playlist['tracks'][track_position]}})
        ...
        user = await SQLiteMusicDB.get_user(user_id)
        ...
        for index, mail in enumerate(inbox.copy()):
            ...
            del inbox[index]
            if is_accept:
                share_playlists = await SQLiteMusicDB.get_user(mail["sender"], d_type="playlist")
                ...
                await SQLiteMusicDB.update_user(mail['sender'], {"$push": {f"playlist.{mail['referId']}.perms.read": user_id}})
                await SQLiteMusicDB.update_user(user_id, {"$set": {
            ...
            await SQLiteMusicDB.update_user(user_id, {"$set": {"inbox": inbox}})
            return payload
...
async def getSettings(bot: commands.Bot, data: Dict) -> Dict:
    ...
    settings = await SQLiteMusicDB.get_settings(guild_id)
    if "dj" in settings:
...
async def updateSettings(bot: commands.Bot, data: Dict) -> None:
    ...
    await SQLiteMusicDB.update_settings(guild.id, {"$set": data})
