...
class Welcome(commands.Cog):
    ...
    @app_commands.command(
        name="setwelcome",
        description="Set the welcome channel and message. Use {user} to mention the new member.",
    )
    @app_commands.describe(channel="Where to post welcome messages", message="e.g. Welcome {user}!")
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        if cfg["welcome_channel_id"] and cfg["welcome_message"]:
            ...
            if channel is not None:
                text = cfg["welcome_message"].replace("{user}", member.mention)
